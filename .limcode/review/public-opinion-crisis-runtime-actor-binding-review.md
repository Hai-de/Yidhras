...
```json
{
  "formatVersion": 4,
  "kind": "limcode.review",
  "reviewRunId": "review-moaihmqx-rcps5f",
  "createdAt": "2026-04-22T00:00:00.000Z",
  "updatedAt": "2026-04-22T20:36:37.856Z",
  "finalizedAt": "2026-04-22T20:36:37.856Z",
  "status": "completed",
  "overallDecision": "needs_follow_up",
  "header": {
    "title": "public-opinion-crisis-runtime-actor-binding-review",
    "date": "2026-04-22",
    "overview": "审查 public_opinion_crisis 世界包在真实运行链中的 actor 绑定缺口，重点核对 pack actor、pack identity 与宿主 inference identity/agent/binding 体系的断裂位置。"
  },
  "scope": {
    "markdown": "# Public Opinion Crisis Runtime Actor Binding Review\n\n- Date: 2026-04-22\n- Scope: `public_opinion_crisis` 世界包在真实运行链中的主体绑定、推理入口与 pack runtime 物化关系\n- Mode: review-only\n\n## 评审目标\n\n本轮只回答一个问题：\n\n> 为什么 `public_opinion_crisis` 已经能进入真实 inference 运行链，但 pack 中声明的 `actor-player` 不能自然成为当前 inference 主体，且其 `actor_state` 不能稳定进入 role prompt？\n\n## 当前已知现象\n\n1. `PackManifestLoader` 与 `parseWorldPackConstitution` 已通过，说明 pack contract 本身可加载。\n2. 独立端口真实 `/api/inference/preview` 已确认运行到了 `world-public-opinion-crisis`，说明 active pack 选择已成立。\n3. `agent_id=actor-player` 会返回 `AGENT_NOT_FOUND`。\n4. `identity_id=system` 能成功进入 inference，但 role prompt 中 `runtime.pack_state.actor_state.*` 大量回落默认值。\n\n## 评审方法\n\n- 逐段核对 `resolveActor()`、`buildPackStateSnapshot()`、`materializer.ts` 与 identity seed/宿主 actor 入口。\n- 每确认一段链路，就追加一个 milestone 和记一条结构性结论。"
  },
  "summary": {
    "latestConclusion": "本次审查确认：`public_opinion_crisis` 世界包已经能通过 schema、manifest loader 与真实 inference pack 选择链进入运行时，说明项目对现实题材世界观的基础容纳性成立；但主体级容纳仍存在系统性缺口。根因不在该包本身，而在宿主 inference 主体系统与 pack actor/pack identity 系统之间没有自动桥接：`resolveActor()` 只接受宿主 runtime agent 或 Prisma identity/binding，`buildPackStateSnapshot()` 又只按 `resolvedAgentId` 回填 actor_state，而 `materializer.ts` 对 `pack.identities` 的处理仅限于 pack runtime world entity，不会同步进入宿主 identity/agent/binding 体系。结果是：world pack 的世界状态可以自然进入 prompt，但 pack 作者声明的玩家 actor 无法自然成为 inference 当前主体，`actor_state` 在真实运行链中难以稳定生效。这应被视为项目级主体桥接空白，而不是单个世界包的实现瑕疵。",
    "recommendedNextAction": "后续若要继续拷打项目，应单独立项审查并设计 `pack actor / pack identity -> inference actor` 的正式桥接策略，至少明确：1）pack actor 是否只是内部语义对象；2）若不是，如何映射到宿主 identity/agent/binding；3）`buildInferenceContext` 与 `buildPackStateSnapshot` 应如何支持 pack 主体的一等视角。",
    "reviewedModules": [
      "apps/server/src/inference/context_builder.ts",
      "apps/server/src/packs/runtime/materializer.ts",
      "apps/server/src/db/seed_identity.ts",
  ...
  "milestones": [
    {
      "id": "M1",
      "title": "主体解析与 pack actor 映射断裂点确认",
      "status": "completed",
      "recordedAt": "2026-04-22T20:35:38.673Z",
      "summaryMarkdown": "已确认真实运行链中的第一处根因断裂不在 pack schema，而在主体解析路径：`resolveActor()` 只接受宿主 `agent_id` 或 Prisma `identity_id` 入口；`buildPackStateSnapshot()` 又只在 `resolvedAgentId` 与 pack entity state `entity_id` 相等时才回填 `actor_state`。与此同时，`materializer.ts` 对 `pack.identities` 的处理仅是把它们物化成 pack world entity，并不会同步进入 Prisma identity / identityNodeBinding / runtime agent 体系；而 `seed_identity.ts` 只预置 `system`、`user-001`、`agent-001/002/003`。因此 `actor-player` 既不是宿主 agent，也没有对应的宿主 identity/binding，导致它不能直接作为 inference `agent_id`，也不会在真实 inference 中自然拿到 `actor_state`。",
      "conclusionMarkdown": "当前项目存在 pack actor 系统与宿主 inference 主体系统分裂：世界包可以声明 actor 与 identity，但 inference 主体解析并不消费这些 pack-level 主体定义。",
      "evidence": [
        {
        ...
        {
          "path": "apps/server/src/db/seed_identity.ts",
          "lineStart": 150,
          "lineEnd": 158
        },
        {
      ...
      "recommendedNextAction": "继续审查是否已有其他桥接层或 operator/runtime 辅助接口能把宿主 identity 绑定到 pack actor；若没有，应把该缺口归类为项目级主体桥接缺陷。",
      "findingIds": [
        "F-pack-actor-与-inference-主体系统分裂",
        "F-javascript-2"
      ]
    },
    {
      "id": "M2",
      "title": "缺少现成桥接面确认：operator 仍只接受宿主 subject entity",
      "status": "completed",
      "recordedAt": "2026-04-22T20:36:22.373Z",
      "summaryMarkdown": "继续追查后，没有发现现成桥接层能把 `pack actor` 或 `pack identity` 直接提升为 inference 主体。相反，现有 operator / advanced contracts 入口仍以宿主 `subjectEntityId` 或 `agent_id` 为前提，例如 `getOperatorAdvancedContracts()` 直接调用 `buildInferenceContextV2(context, { agent_id: subjectEntityId, strategy: 'mock' })`。与此同时，`active_pack_runtime_facade.resolvePackVariables()` 也只暴露 `pack` 与极薄的 `runtime.current_tick`，并不提供 pack actor 绑定辅助。这进一步说明当前系统默认假设“主体”来自宿主 runtime，而不是 world pack 声明。",
      "conclusionMarkdown": "项目里没有现成的 pack actor -> inference actor 桥接面；现有高级观察与调试接口也沿用宿主 subject/agent 入口，因此该缺口是系统性设计结果，而不是单点遗漏。",
      "evidence": [
        {
        ...
        {
          "path": "apps/server/src/core/active_pack_runtime_facade.ts",
          "lineStart": 93,
          "lineEnd": 127,
          "symbol": "resolvePackVariables"
        }
      ],
      "reviewedModules": [
        "apps/server/src/app/services/operator_contracts.ts",
      ...
      "recommendedNextAction": "收束本次 review：当前已足够说明缺口属于系统性主体桥接空白，可形成最终审查结论。",
      "findingIds": [
        "F-缺少-pack-actor-桥接辅助接口"
      ]
    }
  ],
  "findings": [
    {
      ...
      "severity": "high",
      "category": "javascript",
      "title": "pack actor 与 inference 主体系统分裂",
      "descriptionMarkdown": "`resolveActor()` 只接受宿主 runtime agent 或 Prisma identity/binding 作为 inference 主体入口，而 `buildPackStateSnapshot()` 又只按 `resolvedAgentId` 去匹配 pack entity core state。world pack 中声明的 `entities.actors` 与 `identities` 没有自动桥接进宿主主体系统，因此像 `actor-player` 这样的 pack actor 无法自然成为 inference 当前主体，也无法稳定拿到 `actor_state`。",
      "recommendationMarkdown": "明确项目对 pack actor / pack identity 的定位：要么声明它们只是 pack 内部语义对象；要么补一个正式桥接层，把 pack actor / identity 映射到宿主 inference 主体系统。",
      "evidence": [
        {
          "path": "apps/server/src/inference/context_builder.ts",
          "lineStart": 151,
    ...
    },
    {
      ...
      "severity": "high",
      "category": "javascript",
      "title": "pack.identities 未进入宿主 identity 体系",
      "descriptionMarkdown": "`materializer.ts` 对 `pack.identities` 的处理仅是将其物化为 pack runtime 中的 `abstract_authority` world entity，而不会创建 Prisma identity、runtime agent 或 identityNodeBinding。与此同时，`seed_identity.ts` 只预置固定宿主 identity/agent 集，导致 pack 作者声明的 identity 并不能被 inference 主体解析直接消费。",
      "recommendationMarkdown": "如果 pack 级 identity 预期参与真实 inference，应补明确的宿主同步/桥接机制；否则应在 contract 和文档中明确 `pack.identities` 只是 pack 内部治理对象，而不是 inference 可用 identity。",
      "evidence": [
        {
          "path": "apps/server/src/packs/runtime/materializer.ts",
          "lineStart": 156,
    ...
    },
    {
      ...
      "title": "缺少 pack actor 桥接辅助接口",
      "descriptionMarkdown": "继续审查后，没有发现现成的辅助接口能把 world pack 中声明的 actor/identity 解析成 inference 主体。相反，operator contracts 和调试入口继续以宿主 `agent_id` / `subjectEntityId` 为唯一主体入口，说明 pack actor 绑定缺口不是偶发遗漏，而是当前接口面整体未覆盖的设计空白。",
      "recommendationMarkdown": "若项目希望 world pack 成为真正的一等内容单元，需要增加一层正式的 actor bridge/lookup surface，让 pack actor、pack identity 与宿主 inference 主体系统之间存在可解释的映射与诊断能力。",
      "evidence": [
        {
          "path": "apps/server/src/app/services/operator_contracts.ts",
          "lineStart": 61,
  ...
  "render": {
    "rendererVersion": 4,
    "bodyHash": "sha256:5c2dc56f481f99d77f72bd5ebd4fe4411298f26196fb3ba4d911d08e31a31de2",
    "generatedAt": "2026-04-22T20:36:37.856Z",
    "locale": "zh-CN"
  }
}
```
