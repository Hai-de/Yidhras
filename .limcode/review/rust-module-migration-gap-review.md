# Rust 模块迁移剩余 TS 逻辑与缺口审查
- 日期: 2026-04-22
- 概述: 审查当前项目中已切换/部分切换到 Rust 的模块，识别仍未完成迁移的 TS 逻辑、Rust 侧缺失能力，以及导致 TS 代码仍然必须保留的边界原因。日期：2025-02-14
- 状态: 已完成
- 总体结论: 需要后续跟进
...
- 日期：2025-02-14
- 目标：审查当前项目中已切换/部分切换到 Rust 的模块，识别仍未完成迁移的 TS 逻辑、Rust 侧缺失能力，以及导致 TS 代码仍然必须保留的边界原因。
...
- 最新结论: 当前项目中，已切 Rust 的模块大多只迁移了“纯计算/纯 session 内核”，而没有迁移“宿主运行时 ownership”。从完成度上看：scheduler decision kernel 与 memory trigger engine 的算法核心基本已迁移到 Rust，但 TS 仍必须保留作为 fallback 与 parity 基线，同时继续承担数据库副作用、上下文构造、结果落地与 worker/runtime 编排。world engine 的迁移最不彻底：Rust sidecar 已有 pack session、prepare/commit/abort、query、objective execution 骨架，但 pack runtime persistence、宿主时钟同步、plugin contributor 扩展体系、query host seam、invocation side effects 仍显著依赖 TS。结论是：项目并不是“还有零散 TS 逻辑没切”，而是整体采用了“Rust sidecar core + TS host orchestration”的阶段性架构；其中 world engine 仍存在较大的未迁移宿主能力缺口，是 TS 无法下线的首要原因。
...
- 说明:
  ...
  Rust sidecar prepare/commit 只维护 sidecar session 内状态，真正写入 pack runtime storage 的 `upsert_entity_state` / `append_rule_execution` / `set_clock` 处理仍由 TS `world_engine_persistence.ts` 完成，而且 clock delta 只被提取没有完全统一成 Rust-owned host clock。因此 world engine 尚未真正拥有 runtime persistence 与宿主状态推进。
- 建议:

  若目标是 Rust 主导 world runtime，需要优先明确 persistence ownership：是继续 host-apply delta，还是将 pack runtime storage 写入也迁到 Rust。未明确前，TS 无法删除。
- 证据:
...
- 说明:
  ...
  TS 中定义了 `StepContributor` / `RuleContributor` / `QueryContributor` registry，并由 plugin runtime 注册扩展能力；Rust sidecar 当前没有对应 contributor bridge，因此任何依赖 pack-local plugin 的 world engine 扩展都必须继续经过 TS 宿主体系。
- 建议:

  在 world engine 宣称迁移完成前，需要先决定 plugin contributor 是继续留在 TS host，还是建立 Rust sidecar 可消费的扩展协议。
- 证据:
...
- 说明:
  ...
  虽然 Rust sidecar 可响应 `world.state.query` 和 `world.rule.execute_objective`，但项目实际查询数据很多仍由 TS `createPackHostApi` 直连 repository 提供；objective sidecar 返回的 mutation/event plan 也仍由 TS enforcement engine 应用到 entity state 与 event bridge。Rust 当前没有接手完整 query host seam 和 invocation side effects。
- 建议:

  需要分别梳理 query seam 与 invocation side-effect seam，确认哪些必须长期保留在 TS，哪些属于 Rust phase2/phase3 缺口。
- 证据:
...
- 结论:
  ...
  World engine 是当前迁移最不彻底的部分：Rust 侧已有协议骨架和 session 核心，但项目真正依赖的持久化、宿主时钟、插件贡献、查询宿主桥接和执行落地仍高度依赖 TS。
- 证据:
...
```json
{
  ...
  "header": {
    "title": "Rust 模块迁移剩余 TS 逻辑与缺口审查",
    "date": "2026-04-22",
    "overview": "审查当前项目中已切换/部分切换到 Rust 的模块，识别仍未完成迁移的 TS 逻辑、Rust 侧缺失能力，以及导致 TS 代码仍然必须保留的边界原因。日期：2025-02-14"
  },
  "scope": {
    "markdown": "# Rust 模块迁移剩余 TS 逻辑与缺口审查\n\n- 日期：2025-02-14\n- 目标：审查当前项目中已切换/部分切换到 Rust 的模块，识别仍未完成迁移的 TS 逻辑、Rust 侧缺失能力，以及导致 TS 代码仍然必须保留的边界原因。\n- 范围：优先覆盖 world engine、scheduler decision kernel、memory trigger engine，以及它们在 server runtime 中的宿主协调层。\n\n## 当前结论\n\n- 审查进行中。\n\n## 已审查模块\n\n- 暂无\n\n## 发现\n\n- 暂无\n\n## 建议后续动作\n\n- 分模块核对：接口边界、fallback 路径、runtime loop/clock/持久化宿主职责、TS 与 Rust 的重复实现范围。"
  },
  "summary": {
  ...
  "milestones": [
    ...
    },
    {
      ...
      "conclusionMarkdown": "World engine 是当前迁移最不彻底的部分：Rust 侧已有协议骨架和 session 核心，但项目真正依赖的持久化、宿主时钟、插件贡献、查询宿主桥接和执行落地仍高度依赖 TS。",
      "evidence": [
        {
