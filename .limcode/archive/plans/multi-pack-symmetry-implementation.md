...
> 创建: 2026-05-11
> 状态: **已完成**（类型错误 0，1321 测试全通过，AppContext + SimulationManager backward-compat stub 全部移除，死代码清理完成）
...
| (未提交) | 收尾 | Phase 4 完成: 类型错误 43→0，37 文件 pack_runtime_resolution 迁移，snapshot_capture 重写，5 新集成测试文件，文档更新，死代码清理。Backward-compat stub 完整移除: AppContext 5 方法 (?→非可选, applyClockProjection 删除)，SimulationManager 10 @deprecated 方法删除，AppInfrastructure 接口补充，测试全适配 |

总计: **13 次提交**，**~85 文件变更**，**1001 单元 + 320 集成测试全通过**。
