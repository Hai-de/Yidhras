...
```typescript
// apps/server/src/packs/runtime/spatial_runtime.ts
...
interface SpatialRuntime {
  ...
  getLocationState(locationId: string): LocationState | null  // A 层: 地点实体状态（label, public_description, hidden_details, tags）
...
```typescript
// apps/server/src/perception/types.ts
...
| `apps/server/src/packs/runtime/spatial_runtime.ts` | 新增 | SpatialRuntime 实现（邻接图 + BFS 距离 + 实体移动） |
| `apps/server/src/perception/` | 新增目录 | PerceptionResolver 接口 (`types.ts`) + 默认 `spatial_proximity` 实现 (`default_resolver.ts`) |
| `apps/server/src/app/runtime/perception_pipeline.ts` | 新增 | 感知管线实现（sim loop 第 6 步） |
| `apps/server/src/core/simulation.ts` | 修改 | 持有 SpatialRuntime |
| `apps/server/src/app/runtime/PackSimulationLoop.ts` | 修改 | 增加第 6 步（感知管线）。注：旧版 `simulation_loop.ts` 已于多 worker 重构中删除，感知管线迁入 `PackSimulationLoop` |
| `apps/server/src/context/sources/spatial_proximity.ts` | 修改 | 注入 `public_description` + 调查门控 `hidden_details` |
| `apps/server/src/context/source_registry.ts` | 修改 | 注册空间上下文源；传入 investigation 查询能力 |
| `apps/server/src/context/service.ts` | 修改 | 增加 `getInvestigatedLocations()` 查询，传入 adapter input |
| `apps/server/src/context/source_registry.ts` | 修改 | `ContextSourceAdapterInput` 新增 `investigated_location_ids` 字段 |
| `apps/server/src/packs/schema/constitution_schema.ts` | 修改 | location entity state schema 新增 `hidden_details` 可选字段 |
| `apps/server/src/packs/runtime/spatial_runtime.ts` | 修改 | 新增 `getLocationState()` 方法 |
| `data/world_packs/snowbound_mansion/config.yaml` | 修改 | 拆分 location description → `public_description` + `hidden_details` |
| `apps/server/src/domain/rule/enforcement_engine.ts` | 修改 | 空间谓词支持（`location.in` / `location.adjacent_to`） |
...
### Phase 2: 原型世界包 ✅ (完成于 2026-05-09)

1. [x] 编写 `data/world_packs/snowbound_mansion/config.yaml`
2. [x] 结构化随机角色生成（trait 池 + pick 宏）
3. [x] 端到端验证：加载 → 物化展开 → AI 角色扮演 → 多日模拟
