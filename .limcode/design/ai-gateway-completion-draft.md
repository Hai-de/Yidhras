...
> **实施状态**：Phase 0-3 全部完成（2026-05-10）。详见 `.limcode/plans/ai-gateway-completion-plan.md`。

AI Gateway 架构分层（task → route → gateway → adapter → elasticity）原有以下结构性缺陷，现已全部修复：

- ~~**Provider 单一性**~~ → ✅ 4 个真实 provider adapter（OpenAI、Anthropic、DeepSeek、Ollama），多 provider fallback 链已验证
- ~~**弹性层空转**~~ → ✅ circuit breaker 状态跨请求保持（生命周期修复），rate limiter 含 429 动态校准，多 provider 下 fallback 有效
- ~~**Tool loop 无 token 预算**~~ → ✅ `tiktoken` 精确计数（OpenAI 兼容）+ 字符数/3.5 估算（Anthropic），超阈值提前终止
- ~~**Rate limiter 静态配置**~~ → ✅ `adjustFromHints()` 动态校准：429 → 降级 → 冷却 → 逐步恢复
- ~~**无 streaming**~~ → ✅ adapter 流式接口（`executeStream`）+ gateway 透传 + Anthropic/OpenAI SSE 事件映射；SSE endpoint 待前端接入
- ~~**无缓存**~~ → ✅ LRU 内存缓存 + per-task-type TTL，缓存命中写审计记录（`provider: 'cache'`）
- ~~**Circuit breaker 实例生命周期 bug**~~ → ✅ `cbMap`/`rlMap` 已提升到 `createModelGateway` 闭包作用域

**当前仍缺失**（§4 覆盖的可插拔配置除外）：
- Provider 仍为硬编码 5 个 adapter。新增 OpenAI-compatible 渠道需修改源码 → §4 解决
...
| # | 工作项 | 工作量估算 | 说明 |
|---|--------|-----------|------|
| 0 | ✅ **Circuit breaker / rate limiter 生命周期修复** | 已实施 | cbMap/rlMap 提升到闭包作用域 |
| 1 | ✅ OpenAI 兼容通用 adapter 重构 | 已实施 | `openai_compatible.ts`（~260 行），含 `capabilityOverrides` |
| 2 | ✅ Anthropic provider adapter | 已实施 | `anthropic.ts`（~580 行），含折中 structured output + system prompt 提取 |
| 3 | ✅ DeepSeek provider adapter | 已实施 | `deepseek.ts`（~30 行工厂函数） |
| 4 | ✅ Ollama/local provider adapter | 已实施 | `ollama.ts`（~25 行），含 embedding fallback 注册 |
| 5 | ✅ Provider-level fallback 验证 | 已实施 | 8 个集成测试覆盖 fallback 链 + CB 状态持久 |

### P1 — 高优先级（✅ 全部实施）

| # | 工作项 | 状态 | 说明 |
|---|--------|------|------|
| 6 | ✅ Rate limit 动态校准 | 已实施 | `adjustFromHints()` + adapter 429 header 提取 |
| 7 | ✅ Tool loop token 预算管理 | 已实施 | `token_counter.ts`（tiktoken + 字符估算）+ tool loop 集成 |
| 8 | ✅ Response caching | 已实施 | `cache.ts`（LRU 500 entries + per-task-type TTL）+ 缓存审计 |

### P2 — 中优先级（✅ 实施）

| # | 工作项 | 状态 | 说明 |
|---|--------|------|------|
| 9 | ✅ Streaming / SSE 支持 | 已实施 | adapter executeStream + gateway 透传；SSE endpoint 待前端接入 |
...
> **全部 Phase 已完成（2026-05-10）**。以下为实际实施记录。

```
Phase 0 (前置修复) ✅
  └── Circuit breaker / rate limiter 生命周期修复 — cbMap/rlMap 提升到闭包

Phase 1 (P0, 多 provider) ✅
  ├── openai_compatible.ts 重构 (~260 行)
  ├── anthropic.ts (~580 行) + deepseek.ts (~30 行) + ollama.ts (~25 行)
  ├── 注册表更新 (4 provider + 5 model + fallback 链 + embedding fallback)
  └── 8 个 fallback 集成测试 (含 CB 状态持久验证)

Phase 2 (P1, 弹性与成本) ✅
  ├── Rate limit 动态校准 (adjustFromHints + 3 adapter 适配)
  ├── Token 预算管理 (tiktoken + tool loop 集成)
  └── Response caching (LRU 500 + per-task-type TTL)

Phase 3 (P2, streaming) ✅
  └── Adapter executeStream + gateway 透传 (Chat Completions SSE + Anthropic SSE)
```

**测试结果**：Unit 981 + Integration 239 = 1220，零回归。
...
## 4. 可插拔 Provider 配置文件扩展（✅ 已实施）

> 实施日期：2026-05-10。详见 `.limcode/plans/ai-gateway-completion-plan.md` §Phase 4。

### 4.0 目标与背景

当前 `gateway.ts` 的 adapter 列表是**硬编码**的 5 个：`mock`、`openai`、`anthropic`、`deepseek`、`ollama`。新增任何 provider 都需要修改 TypeScript 源码并重新部署。

实际需求：
- 市面上绝大多数模型服务（OpenRouter、SiliconFlow、Groq、Together、Fireworks、Moonshot、百川、智谱等）都兼容 OpenAI Chat Completions API
- 这些渠道只需不同的 `base_url` + `api_key_env` + 少量 `capabilityOverrides`
- 用户（包括 future self）应该能在不修改代码、不重新编译的情况下，通过 YAML 配置添加新渠道

### 4.1 设计原则

1. **OpenAI-compatible 渠道零代码添加**：配置即注册，无需重启服务（利用现有热加载）
2. **非兼容渠道保留代码 adapter**：Anthropic Messages API、OpenAI Responses API 等仍需代码实现，但通过统一注册表动态挂载
3. **不破坏现有 `ai_models.yaml` 结构**：`providers` / `models` / `routes` 三段保持原语义，新增独立的 `provider_templates` 段
4. **配置合并策略与现有注册表一致**：`BUILTIN_AI_REGISTRY_CONFIG` 为基线，`ai_models.yaml` 做 deep merge（按 `name` key 合并 templates，按 `provider` key 合并 providers）
5. **向后无负担**：项目尚未上线，允许在 schema 层面做 breaking change

### 4.2 配置文件扩展

#### 4.2.1 新增 `provider_templates` 段

```yaml
version: 1

provider_templates:
  # ── 零代码添加 OpenAI-compatible 渠道 ──
  - name: openrouter
    ...
    capability_overrides:
      ...
      supports_seed: true
      max_structured_output: json_schema
    default_headers:
  ...
  - name: siliconflow
    ...
    capability_overrides:
      max_tokens_field: max_tokens
      supports_seed: false
      max_structured_output: json_object
...
models:
  - provider: openrouter
    ...
    capabilities:
      ...
      max_context_tokens: 200000
      max_output_tokens: 8192
    tags: [openrouter, claude, structured]
    availability: active
...
const providerTemplateSchema = z.object({
  ...
  builtin_name: nonEmptyStringSchema.nullish(),
}).strict();

export const aiRegistryConfigSchema = z.object({
  version: z.number().int().positive().default(1),
  provider_templates: z.array(providerTemplateSchema).default([]), // 新增
...
`provider_templates` 的合并逻辑与 `providers` / `models` 保持一致：
...
#### 4.3.1 新增 `adapter_registry.ts`

职责：维护从 `kind` 到 `adapter factory` 的注册表，以及从配置动态构建 adapter 列表。
...
import { createOpenAiCompatibleAdapterFromTemplate, createOpenAiProviderAdapter } from './openai.js';
...
export const buildAdaptersFromRegistry = (registryConfig: AiRegistryConfig): AiProviderAdapter[] => {
  ...
  // 1. 加载内置 adapter（如果 template 未覆盖）
  ...
  for (const template of registryConfig.provider_templates ?? []) {
    if (template.kind === 'openai_compatible') {
      ...
      if (factory) {
        adapters.set(template.name, factory());
      } else {
        // 运行时警告：builtin_name 不存在
...
#### 4.3.2 `openai_compatible.ts` 新增工厂

```typescript
// 新增于 openai_compatible.ts
...
export const createOpenAiCompatibleAdapterFromTemplate = (
  ...
  return createOpenAiCompatibleAdapter({
    ...
    resolveApiKey(input) {
      // 优先级：provider_config.api_key_env > template.api_key_env
...
// 修改后（配置驱动，保留 adapters 参数供测试注入）
export const createModelGateway = ({
  adapters,
  context,
  registryConfig
}: CreateModelGatewayOptions = {}): ModelGateway => {
  const resolvedRegistry = registryConfig ?? getAiRegistryConfig();
  const activeAdapters = adapters ?? buildAdaptersFromRegistry(resolvedRegistry);
  const adapterByProvider = new Map(activeAdapters.map(a => [a.provider, a]));
  // ...
};
```
...
| kind | 实现方式 | 工作量 | 示例 |
|------|---------|--------|------|
| `openai_compatible` | **配置驱动，零代码** | 0 行 | OpenRouter, SiliconFlow, Groq, Together, Fireworks, Moonshot, 通义千问, 本地 vLLM |
| `builtin` | 引用已有代码 adapter | 2 行 | anthropic, openai (Responses API) |
| `azure_openai` | 新增代码 + 注册 factory | 0.5 天 | Microsoft Azure OpenAI（需 token 轮询 + api-version） |
| `vertex_ai` | 新增代码 + 注册 factory | 1 天 | Google Vertex AI (Gemini) |
| `bedrock` | 新增代码 + 注册 factory | 1 天 | AWS Bedrock（需 SigV4 签名） |
| `custom_http` | 新增代码 + 注册 factory | 1-2 天 | 内部私有模型服务 |

新增代码 kind 的接入点：

```typescript
// adapter_registry.ts — 未来扩展
const kindFactories = new Map<string, (template: AiProviderTemplate) => AiProviderAdapter>([
  ['openai_compatible', createOpenAiCompatibleAdapterFromTemplate],
  ['azure_openai', createAzureOpenAiAdapterFromTemplate],
  // ...
]);

export const registerAdapterKind = (
  kind: string,
  factory: (template: AiProviderTemplate) => AiProviderAdapter
): void => {
...
#### D1. `provider_templates` 是否允许覆盖内置 adapter？

当前设计允许 `provider_templates` 中声明 `name: openai` `kind: openai_compatible`，这会**覆盖**内置的 OpenAI adapter（Responses API + Embeddings 路径丢失）。

- **选项 A（允许覆盖，当前草案）**：template 按 name 覆盖。用户若覆盖 `openai` 为 `openai_compatible`，则丧失 Responses API 和 Embeddings 能力，后果自负。
- **选项 B（禁止覆盖内置名称）**：内置名称（`openai`、`anthropic`、`deepseek`、`ollama`、`mock`）在 `provider_templates` 中为保留字，声明时报错或忽略。
- **选项 C（保留字 + 独立命名空间）**：template 的 `name` 必须与内置 adapter 名称不同，若要扩展内置 adapter 的行为，通过 `providers` 段的字段覆盖（如 `default_headers`），不通过 template 覆盖。

**建议**：选项 A 最灵活，但文档中需明确标注"覆盖内置名称会丧失该 adapter 的特殊能力"。如果团队倾向保守安全，选 C。

#### D2. `provider_templates` 与 `providers` 的耦合关系

当前 `providers` 段仍然需要为每个 provider 声明 `enabled`、`api_key_env`、`base_url` 等。这导致 OpenRouter 需要在两个地方写：
...
providers:
  - provider: openrouter
    enabled: true
    api_key_env: OPENROUTER_API_KEY  # 重复
```

- **选项 A（保持分离，当前草案）**：template 描述"如何构建 adapter"，provider config 描述"运行时参数"。两边各自独立，允许 template 声明后不在 providers 中启用（如备用的 template）。
- **选项 B（template 自动生成 provider config）**：如果 `providers` 段不存在同名 provider，自动从 template 生成一个默认 provider config（`enabled: true`，其余字段从 template 继承）。减少重复配置。
- **选项 C（合并为一段）**：把 `provider_templates` 合并进 `providers` 段，在 provider config 中新增 `kind` 和 `capability_overrides` 字段。但这会显著改变 `providers` 的 schema 语义。

**建议**：选项 A 最清晰，但重复配置确实恼人。如果未来 provider 数量 > 10，考虑 B。目前先用 A，后续根据实际 YAML 冗长度迭代。

#### D3. `provider_templates` 是否支持独立 YAML 文件？

当前所有 AI 配置都在 `ai_models.yaml` 中。随着 provider 增多，这个文件可能变得很长。

- **选项 A（单文件，当前草案）**：所有 template 写在 `ai_models.yaml` 中。简单，利用现有 `registry_watcher.ts` 的防抖热加载。
- **选项 B（支持 `ai_providers.d/*.yaml`）**：类似 runtime config 的 `conf.d/` 碎片机制，允许把每个 provider template 放在独立 YAML 文件中。启动时按文件名排序加载并合并。需要扩展 `registry.ts` 的文件加载逻辑。
- **选项 C（world-pack 级 provider 注册）**：允许 world-pack 在 `pack.ai` 中声明 `provider_templates`（类似当前 `pack.ai` 的 `route_hints`）。但 world-pack 不应该影响 AI 基础设施层（provider adapter 是 host-side 能力）。

**建议**：A 足够当前使用。B 可以在 provider 数量 > 10 时实施，是一个低风险的后续优化。

#### D4. `builtin_name` 的 schema 校验层级

`provider_templates` 中 `kind: builtin` 时需要指定 `builtin_name`。这个名称在 zod schema 层面是否做 enum 限制？

- **选项 A（zod enum，编译时校验）**：`builtin_name: z.enum(['mock', 'openai', 'anthropic', 'deepseek', 'ollama'])`。新增内置 adapter 时必须同步修改 schema，确保类型安全。
- **选项 B（运行时检查，当前草案）**：schema 只要求 `nonEmptyStringSchema`，运行时检查 `builtinFactories.has(builtin_name)`。新增内置 adapter 时只需修改 `adapter_registry.ts` 中的 map。

**建议**：A 更严格，但新增 adapter 时需要改 schema + registry + 文档，维护负担稍大。B 更灵活，且错误信息清晰（运行时 warn log）。项目未上线，选 A 还是 B 主要看团队对类型安全的偏好。我倾向 **A**（用 enum 显式约束）。

#### D5. 自定义 adapter kind 的 plugin 化路径

未来如果需要接入不兼容 OpenAI 的私有模型（如公司内部的自研模型），除了改代码之外，是否允许通过 plugin runtime 动态加载 adapter JS 模块？

- **选项 A（当前仅代码注册）**：自定义 adapter 必须修改 `adapter_registry.ts` 并重新部署。这是最安全的路径。
- **选项 B（预留 plugin 接口）**：在 `registerAdapterKind` 基础上，允许 plugin 通过 `host.registerAdapterKind()` 在 runtime 注册自定义 factory。但这意味着 AI Gateway 的执行路径可以被 plugin 代码注入，需要 sandbox 和权限管控。
- **选项 C（外部 HTTP 代理）**：对私有模型不实现本地 adapter，而是要求它暴露一个 OpenAI 兼容的 HTTP 代理（如通过 litellm、one-api 等），然后用 `kind: openai_compatible` 接入。零代码且隔离。

**建议**：C 是最实际的路径（大多数私有模型都通过 one-api/litellm 做兼容层转接）。B 的 sandbox 复杂度不值得在当前阶段投入。当前设计为 A，但文档中推荐 C 作为推荐实践。

#### D6. `provider_templates` 的流式能力声明

当前 `createOpenAiCompatibleAdapterFromTemplate` 创建的 adapter 自动继承流式能力（`executeStream` 在 `openai_compatible.ts` 中已实现）。但如果某个 provider 的流式实现有差异（如不支持 `stream_options: { include_usage: true }`），当前 `capabilityOverrides` 没有字段来禁用流式或覆盖流式行为。

- **选项 A（默认全部支持流式，出问题再修）**：目前所有主流 OpenAI-compatible provider 都支持标准 SSE streaming。先不增加复杂度。
- **选项 B（新增 `supportsStreaming: boolean` override）**：在 `capabilityOverrides` 中新增字段，不支持时 gateway 的 `executeStream` 退化到非流式调用。
- **选项 C（新增 `streamingOverrides` 独立段）**：更细粒度地控制流式参数（如 `include_usage`、`stream_format` 等）。

**建议**：A 对当前够用。如果后续接入某个 provider 的流式实现有 bug，再追加 B。

### 4.6 实施步骤

```
Phase 0 (前置，0.5 天)
  └── 0. Circuit breaker / rate limiter 生命周期修复（原有 P0）

Phase 1a (P0 扩展，1 天)
  ├── 1a. OpenAI 兼容通用 adapter 重构（原有）
  ├── 1b. 新增 adapter_registry.ts + buildAdaptersFromRegistry()
  ├── 1c. 新增 createOpenAiCompatibleAdapterFromTemplate()
  ├── 1d. registry.ts 新增 provider_templates schema + 合并逻辑
  ├── 1e. gateway.ts 改为从 registry 动态构建 adapters
  └── 1f. 内置注册表保留 mock/openai/anthropic/deepseek/ollama，provider_templates 为空

Phase 1b (P0 验证，0.5 天)
  ├── 2a. 通过 ai_models.yaml 零代码添加一个测试 provider（如 groq 或本地 mock-compatible）
  ├── 2b. 热加载验证：修改 ai_models.yaml → 新 provider 立即生效（不重启）
  └── 2c. 集成测试：fallback 链包含 template 动态添加的 provider

Phase 2 (后续，按需)
  ├── 3. 新增 `kind: azure_openai` / `kind: vertex_ai` 等代码 adapter
  └── 4. 支持 provider_templates 独立 YAML 文件（ai_providers.d/）
```

### 4.7 与原有设计的衔接

- **与 §2.2（OpenAI 兼容 Provider 通用 Adapter）衔接**：`openai_compatible.ts` 的重构是 §4 的前提。`createOpenAiCompatibleAdapter()` 工厂函数必须先存在，才能被 `createOpenAiCompatibleAdapterFromTemplate()` 调用。
- **与 §2.3（Provider-level Fallback 验证）衔接**：fallback 验证需要覆盖 template 动态添加的 provider，确保 `buildAdaptersFromRegistry()` 构建的 adapter 与硬编码 adapter 在 gateway 层行为完全一致。
- **与 §2.7（Streaming）衔接**：`openai_compatible.ts` 的流式实现自动覆盖所有 `kind: openai_compatible` 的 template provider。
- **与 §3（实施顺序）衔接**：Phase 1a 插入到原有 Phase 1 之后，作为"多 provider 适配"的收尾步骤。
