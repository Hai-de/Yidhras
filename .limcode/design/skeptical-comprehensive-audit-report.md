...
**实际证据**：
- `apps/server/rust/world-engine/src/` 下 8 个 Rust 文件，总计 **1,673 行**
- `apps/server/rust/scheduler-decision/src/` 下 6 个文件，总计 **793 行**
- `apps/server/rust/memory-trigger/src/` 下 8 个文件，总计 **1,981 行**
...
**Evidence**：
- `world-engine_client.ts`：419 行 TS 代码，通过 stdio JSON-RPC 与 Rust 进程通信
- Rust world-engine：1,673 行
- Rust scheduler-decision：793 行
- Rust memory-trigger：1,981 行
...
更重要的是，**三个 Rust sidecar 是独立的进程**。world-engine、scheduler-decision、memory-trigger 各自 spawn。
...
**Coverage 盲点**：
- Rust sidecar 的测试覆盖率未知（无独立 `tests/` 目录，但 `memory-trigger` 中有 4 处 `#[cfg(test)]` 内联测试；`world-engine` 和 `scheduler-decision` 未找到测试代码）
...
- `apps/server/rust/` Rust sidecar 源码（~4,500 行，`memory-trigger` 含 4 处 `#[cfg(test)]` 内联测试）
