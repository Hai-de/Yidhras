...
| Rust World Engine Sidecar | "已完成 sidecar-only 收口" | 约 1,673 行 Rust | 8 | 一个"世界引擎"只有 1,673 行？step/session/state/objective 全在这 |
| AI Gateway | "分层内部管道" | 仅 OpenAI 适配器 | 7 | 无 Anthropic、无本地模型、无多模态 |
...
| 空间语义 Spatial | "A 层 Phase 1 全部完成" | B/C 层为零 | 7 | A 层完成但前端无地图、无 move intent 解析 |
| 多轮对话 Multi-Turn | "设计文档 50KB" | Tag 系统未激活 | 9 | 设计极详尽，核心功能推迟 |
| 前端 Web | "Nuxt 4 + Vue 3" | ~5,300 行 Vue | 7 | 纯管理后台，无实时交互，无游戏画面 |
...
**实际证据**：
- Provider adapters：`apps/server/src/ai/providers/` 下只有 `mock.ts`、`openai.ts`、`gateway_backed.ts`
- `gateway_backed.ts` 不是独立 provider，而是 gateway 的自引用包装
- **实际可落地的 provider 只有 OpenAI**

**怀疑性分析**：

文档详细描述了 circuit breaker、rate limiter、exponential backoff、tool calling、cross-agent bridge 等机制。但这些弹性层是**无本之木**——它们保护的是通往 OpenAI 的单一路径。当 OpenAI API 不可用时，系统没有 Anthropic Claude、没有 Google Gemini、没有本地 Ollama/LM Studio、没有 Azure OpenAI 的替代路径。`route_resolver.ts` 理论上支持多 route，但 `ai_models.yaml` 默认只配置 OpenAI。

更关键的是，文档自己承认（`AI_GATEWAY.md` §5）：**"公开 inference contract 仍然只稳定承诺 mock 与 rule_based"**。这意味着：
- 所有通过 OpenAI 的真实 AI 推理都是"内部/受控能力"
...
**结论**：AI Gateway 是一个过度设计的**OpenAI 客户端包装器**。其弹性层、工具调用、跨 agent 桥接等功能在只有一个 provider 的架构下无法展现真正的价值。如果 OpenAI 被封禁、涨价或降级，系统立即退化为 mock/rule_based 模式。
...
- **Tag 系统 "schema 已就位，用途尚在讨论中，待决定后激活"**（`TODO.md` §提示词流水线升级）

**怀疑性分析**：

5 个 profile 中，真正在生产路径上运行的可能只有前 3 个。`chat-*` profile 的存在更像是为了展示"我们考虑了多轮对话"，但实际上：
- `conversation_profile` 字段在 `InferenceContext` 中有定义，但如何被设置、如何被持久化、如何在多轮之间保持状态，代码中缺乏完整链路
- 多轮对话设计文档（`.limcode/design/multi-turn-conversation-design.md`，50,752 字节）极其详尽，涵盖了 conversation entry、turn、thread、branching、archival 等概念，但 `TODO.md` 明确说 Tag 系统"待决定后激活"
- **一个 50KB 的设计文档，其核心机制（Tag）尚未激活**
...
7. **Action dispatch 支持 move intent**：**部分完成**。dispatcher 能处理 `move` intent，但 AI 无法自然生成 `move` intent——因为"AI → intent 的解析后续由 prompt 工程处理"。这意味着：**空间移动功能在基础设施层面就绪，但在 AI 行为层面不存在**。AI 不知道"走向厨房"应该生成 `move` intent。
8. **Enforcement engine 空间谓词**：完成。预过滤已实现。

**第 7 点是致命缺口**。一个空间模拟系统，AI agent 无法自主决定移动，这意味着空间语义只是**被动的装饰**。AI 可以"读到"自己在厨房，但无法"决定"去客厅。这个关键链接被标记为"prompt 工程后续处理"，即**未开始**。
...
**结论**：空间语义 A 层是**基础设施完备但行为缺失**的典型。所有"系统侧"功能已完成，但"AI 侧"关键链路（move intent 接地）未实现，前端无可视化，原型世界包无端到端验证结果。这是一个**只能演示、不能运行**的子系统。
...
双 provider 支持在**接口层面**存在，但 Prisma schema 的同步维护是一个隐性债务。`schema.sqlite.prisma`、`schema.pg.prisma` 和 `schema.prisma`（第三个文件？）需要保持模型定义一致。任何 schema 变更需要手动同步三个文件，没有自动化校验工具（如 diff check in CI）。
...
- `ai/route_resolver.ts`：route 选择
- `ai/gateway.ts`：dispatch 层
- `ai/providers/openai.ts`：599 行，唯一真实 provider
...
**Provider 单一性**：系统声称支持路由到不同 model 和 provider，但唯一真实的 provider adapter 是 OpenAI。这意味着：
- 如果用户没有 OpenAI API key，系统只能运行 mock/rule_based 模式
- 没有国内可用路径（无 DeepSeek、无 Moonshot、无通义千问适配器）
- 没有本地/私有部署路径（无 Ollama、无 vLLM、无 text-generation-inference）
- 没有多模态路径（OpenAI adapter 中有 `image_url` 和 `file_ref` 的代码，但没有实际的多模态 task 定义）

**Circuit Breaker 的有效性**：circuit breaker 防止对已失败 provider 的持续调用。但系统中只有一个 provider（OpenAI），circuit breaker 的状态机只是把这个单一 provider 标记为 open/closed。当 OpenAI 被标记为 open 时，系统退化为 mock/rule_based。这不是"优雅降级"，这是**功能丧失**。
...
**Gaps**：
- [ ] 非 OpenAI provider 适配器（Anthropic、Google、Azure、本地模型）
- [ ] 国内模型生态支持（DeepSeek、Moonshot、Qwen 等）
...
**Verdict**：AI Gateway 是一个**为单一 provider 设计的过度工程化系统**。其分层（task → route → gateway → adapter → elasticity）在概念上正确，但在只有 OpenAI 的现实下，大部分分层是**空转**的。真正缺失的是**多 provider 生态**和**成本控制能力**。
...
**Conversation History Track**：`conversation_history_track.ts` 读取 `context.agent_conversation_memory` 并生成 section drafts。但 `agent_conversation_memory` 从哪里来？如果这是多轮对话的上下文，它需要在前一轮推理结束时被持久化，并在下一轮被读取。这个持久化路径在哪里？`TODO.md` 说 Tag 系统未激活，而 Tag 是多轮对话组织的关键机制。
...
**5,300 行 Vue 是什么水平？** 作为一个对比，一个中等复杂度的 SaaS 管理后台（如 Supabase 的 dashboard）通常有 50,000-100,000 行前端代码。Yidhras 的前端只有 5,300 行，这意味着：
- 每个页面平均 160 行（overview.vue 270 行已经是最复杂的页面之一）
- 没有复杂的交互组件
- 没有实时协作功能
- 没有游戏化的可视化
...
**Verdict**：前端是一个**轻量级的管理后台**，不是**叙事模拟的交互界面**。其功能仅限于监控和配置，缺乏让操作员"进入世界"的沉浸感。5,300 行代码量证明了这一点。
...
**Agent 自主行为权限**：`resolveSubjectForAgentAction()` 的逻辑是：如果 Agent 有控制 Operator，以 Operator 的 identity 校验 capability；否则以 agent 自身为 subject。这引入了一个微妙的漏洞：如果 Operator A 控制 Agent X，而 Agent X 的自主行为触发了需要高 capability 的操作，系统会以 Operator A 的权限来校验。这意味着：
- Agent X 的行为间接获得了 Operator A 的所有 capability
- 如果 Agent X 被 prompt injection 攻击（例如 AI 被诱导生成恶意 intent），它可以在 Operator A 的权限范围内执行操作
...
**宏系统的实际限制**：`snowbound_mansion/config.yaml` 的注释明确承认："pick 宏的 from 参数仍使用逗号分隔字符串（macro handler 签名限定 args 为 `Record<string, string>`）"。这意味着即使在这个"已完成"的宏系统中，**参数类型只有字符串**。无法传递数字、布尔值或数组。`TODO.md` 中的技术债务 #3 也指出 `variables` schema 不支持数组。这与设计文档中声称的"带类型输出：number / string / string[]"矛盾。
...
- [ ] 宏系统的真类型支持（number、boolean、array）
- [ ] 第三方世界包的兼容性验证工具
- [ ] 世界包市场/分发机制
- [ ] 世界包版本升级路径（config.yaml 结构变更时如何迁移旧包？）
- [ ] 世界包性能基准（加载时间、内存占用、推理延迟）

**Verdict**：世界包系统的基础设施（schema、loader、materializer、scaffold）已完成，但**内容生态尚未起步**。原型世界包缺乏自动化验证，宏系统有类型限制，这些都是阻碍第三方作者入场的障碍。
...
**Coverage 盲点**：
- Rust sidecar 的测试覆盖率未知（Rust 目录下没有 `tests/` 目录）
...
- Rust 代码总量 < 4,500 行，而 TS 代码量（仅 `apps/server/src/`）超过 100,000 行
...
| 稳定参考文档（`docs/`） | ~200 KB |
| 后端 TS 源代码（`apps/server/src/`） | ~150,000 行 |
| 前端 Vue/TS 源代码（`apps/web/`） | ~30,000 行 |
| Rust 源代码 | ~4,500 行 |

文档总量约 630 KB，代码量约 180,000 行。比例看似合理，但**文档的详细程度远超代码的成熟程度**。多轮对话设计文档 50 KB，但 Tag 系统未激活。插槽函数设计 47 KB，但 WASM 沙箱推迟。模板引擎统一设计 49 KB，但双重模块设置推迟。
...
- AI agent 能自主使用空间功能？否（move intent 接地未实现）。
- 前端能可视化空间？否。
...
- variables schema 不支持数组
- 插件 Host API 无 AI 推理接口

这种标记创造了一个**虚假的进度感**："我们知道这些问题，但它们不紧急"。但实际上，这些问题中的每一个都会在未来成为阻塞性障碍。例如，variables 不支持数组意味着所有世界包的配置必须使用逗号分隔字符串，这会在世界包生态扩大时成为兼容性噩梦。
...
### 代码（被验证的实现证据）
- `apps/server/src/` 全部 TS 源码（~415 个文件）
- `apps/server/rust/` Rust sidecar 源码（~4,500 行）
- `apps/web/` 前端源码（~5,300 行 Vue）
- `apps/server/prisma/schema.sqlite.prisma`
- `apps/server/tests/` 全部测试（980 unit + 227 integration）
