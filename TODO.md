...
- [x] 已经开发了不少功能，是时候更新一下api接口和docs/API.md文档了，前端很长一段时间基本没有更新，到时候基本是大翻新，在大翻新前可以升级或者重构对外暴露的api接口
    - API.md 已重写为与实际代码一致（2026-05-01），路由分为 Global (`/api/...`) 和 Pack-scoped (`/:packId/api/...`) 两层，experimental 路由保留并标明不承诺稳定 contract。API 代码层面无需重构。
