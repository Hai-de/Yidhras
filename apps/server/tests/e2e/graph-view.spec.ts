...
const ensureGraphFixtures = async (prisma: PrismaClient) => {
...
describe('graph view e2e', () => {
  it('returns graph projections with filters, search, root selection and validation errors', async () => {
    ...
    await ensureGraphFixtures(kit.prisma);
    try {
      await kit.startServer(3106);
      ...
      const graphViewResponse = await requestJson(kit.baseUrl, '/api/graph/view');
      ...
      const kindsResponse = await requestJson(kit.baseUrl, '/api/graph/view?kinds=relay&kinds=container');
      ...
      const noUnresolvedResponse = await requestJson(kit.baseUrl, '/api/graph/view?include_unresolved=false');
      ...
      const rootResponse = await requestJson(kit.baseUrl, '/api/graph/view?root_id=agent-001&depth=1');
      ...
      const searchResponse = await requestJson(kit.baseUrl, '/api/graph/view?search=relay');
      ...
      const qAliasResponse = await requestJson(kit.baseUrl, '/api/graph/view?q=relay');
      ...
      const inactiveResponse = await requestJson(kit.baseUrl, '/api/graph/view?include_inactive=true');
      ...
      const treeResponse = await requestJson(kit.baseUrl, '/api/graph/view?view=tree');
      ...
      const invalidKindsResponse = await requestJson(kit.baseUrl, '/api/graph/view?kinds=unknown_kind');
      ...
      const invalidDepthResponse = await requestJson(kit.baseUrl, '/api/graph/view?depth=abc');
