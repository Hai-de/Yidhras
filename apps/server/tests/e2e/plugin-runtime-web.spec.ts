...
describe('plugin runtime web e2e', () => {
  it('returns canonical runtime web bundle URLs and rejects missing assets for packs without enabled plugins', async () => {
    const kit = await E2ETestKit.create({ port: 3110, packRef: 'death_note' });
    try {
      await kit.startServer();
      const runtimeResponse = await requestJson(kit.baseUrl, '/api/packs/world-death-note/plugins/runtime/web');
