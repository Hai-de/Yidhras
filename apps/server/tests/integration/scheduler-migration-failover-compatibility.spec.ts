...
describe('scheduler migration failover compatibility integration', () => {
  ...
  it('keeps lease expiry semantics compatible with ownership migration handoff', async () => {
    ...
    const workerALease = await acquireSchedulerLease(kit.context, {
      workerId: 'worker-a',
      partitionId: 'p2',
      now: 1000n,
      leaseTicks: 2n
    ...
    await createSchedulerOwnershipMigration(kit.context, {
      partitionId: 'p2',
      toWorkerId: 'worker-b',
      reason: 'migration-failover compatibility'
    }, TEST_PACK_ID);

    const workerAStillAllowed = await isWorkerAllowedToOperateSchedulerPartition(kit.context, {
      partitionId: 'p2',
      workerId: 'worker-a'
    }, TEST_PACK_ID);
    const workerBAllowed = await isWorkerAllowedToOperateSchedulerPartition(kit.context, {
      partitionId: 'p2',
      workerId: 'worker-b'
    }, TEST_PACK_ID);
    ...
    const beforeExpiryWorkerBLease = await acquireSchedulerLease(kit.context, {
      workerId: 'worker-b',
      partitionId: 'p2',
      now: 1001n,
      leaseTicks: 2n
    ...
    const failoverToWorkerB = await acquireSchedulerLease(kit.context, {
      workerId: 'worker-b',
      partitionId: 'p2',
      now: 1003n,
      leaseTicks: 3n
    ...
    await completeActiveSchedulerOwnershipMigration(kit.context, {
      partitionId: 'p2',
      toWorkerId: 'worker-b'
    }, TEST_PACK_ID);
    ...
    const assignment = await getSchedulerPartitionAssignment(kit.context, 'p2', TEST_PACK_ID);
    expect(assignment?.worker_id).toBe('worker-b');
    expect(assignment?.status).toBe('assigned');

    const cursor = await getSchedulerCursor(kit.context, 'p2', TEST_PACK_ID);
    expect(cursor).not.toBeNull();
    expect(cursor?.last_scanned_tick).toBe(1003n);
    expect(cursor?.last_signal_tick).toBe(1002n);

    const workerASnapshot = await resolveSchedulerOwnershipSnapshot(kit.context, {
      workerId: 'worker-a',
      bootstrapPartitionIds: []
    }, TEST_PACK_ID);
    const workerBSnapshot = await resolveSchedulerOwnershipSnapshot(kit.context, {
      workerId: 'worker-b',
      bootstrapPartitionIds: []
    }, TEST_PACK_ID);
