...
const main = async () => {
  ...
  try {
    ...
    assert(
      isRecord(claimReplay.result.decision) && claimReplay.result.decision.action_type === 'claim_death_note',
      'first pack rule should materialize claim_death_note'
    );
    ...
    assert(
      isRecord(intentReplay.result.decision) &&
        intentReplay.result.decision.action_type === 'form_murderous_intent',
      'second pack rule should materialize form_murderous_intent decision'
    );
