...
fn build_keyword_haystacks(
    ...
    let selected_fields =
        fields.cloned().unwrap_or_else(|| vec![MemoryKeywordFieldDto::ContentText]);
    ...
    for field in selected_fields {
        match field {
            ...
            MemoryKeywordFieldDto::RecentTraceReasoning => {
                if let Some(records) =
                    context.recent.as_ref().and_then(|recent| recent.trace.as_ref())
                {
                    for trace in records {
                        if let Some(reasoning) =
                            trace.payload.get("reasoning").and_then(Value::as_str)
                        {
            ...
            MemoryKeywordFieldDto::RecentEventText => {
                if let Some(records) =
                    context.recent.as_ref().and_then(|recent| recent.event.as_ref())
                {
                    for event in records {
                        let title =
                            event.payload.get("title").and_then(Value::as_str).unwrap_or_default();
    ...
    haystacks.into_iter().filter(|value| !value.trim().is_empty()).collect()
}
...
fn evaluate_keyword_trigger(
    ...
    let matches_keyword = |keyword: &str| {
        haystacks.iter().any(|haystack| contains_keyword(haystack, keyword, case_sensitive))
    };
...
fn resolve_recent_source_records<'a>(
    ...
    match source {
        MemoryRecentSourceDto::Trace => {
            context.recent.as_ref().and_then(|recent| recent.trace.as_deref()).unwrap_or(&[])
        }
        MemoryRecentSourceDto::Intent => {
            context.recent.as_ref().and_then(|recent| recent.intent.as_deref()).unwrap_or(&[])
        }
        MemoryRecentSourceDto::Event => {
            context.recent.as_ref().and_then(|recent| recent.event.as_deref()).unwrap_or(&[])
        }
    }
}
...
fn evaluate_recent_source_trigger(
    ...
    resolve_recent_source_records(context, source).iter().any(|candidate| {
        let current = candidate.payload.get(match_field);
        match op {
            MemoryRecentSourceMatchOpDto::Eq => current == value,
            MemoryRecentSourceMatchOpDto::In => {
                current.is_some_and(|item| values.is_some_and(|items| items.contains(item)))
            }
            MemoryRecentSourceMatchOpDto::Contains => current
                .and_then(Value::as_str)
                .zip(value.and_then(Value::as_str))
                .is_some_and(|(current, expected)| current.contains(expected)),
...
pub fn evaluate_trigger(
    ...
    match trigger {
        MemoryTriggerDto::Keyword { r#match, keywords, case_sensitive, fields, .. } => {
        ...
        MemoryTriggerDto::RecentSource { source, r#match, .. } => evaluate_recent_source_trigger(
            context,
            source,
            &r#match.field,
            &r#match.op,
...
mod tests {
    ...
    use crate::models::{
        ...
        MemoryRecentSourceMatchOpDto, MemoryRecentSourceRecordDto,
        MemoryRecentSourceTriggerMatchDto, MemoryRecentSourcesDto, MemoryRetentionRuleDto,
        MemoryTriggerDto,
    };
    ...
    fn sample_behavior(
        triggers: Vec<MemoryTriggerDto>,
        mode: MemoryActivationModeDto,
    ) -> MemoryBehaviorDto {
        MemoryBehaviorDto {
                ...
                event: Some(vec![MemoryRecentSourceRecordDto {
                    ...
                    payload: Map::from_iter(vec![
                        (
                            ...
                            Value::String("suspicious_death_occurred".to_string()),
                        ),
    ...
    fn always_mode_short_circuits_to_always_match() {
        ...
        assert_eq!(
            compute_matched_triggers(&sample_block(), &behavior, &sample_context()),
            vec![("always".to_string(), 1.0)]
        );
    }
    ...
    fn semantic_trigger_matches_above_threshold() {
        let block = MemoryBlockDto { embedding: Some(vec![0.8, 0.6, 0.1, 0.3]), ..sample_block() };
        let mut context = sample_context();
        context.query_embedding = Some(vec![0.7, 0.65, 0.15, 0.25]);

        let trigger = MemoryTriggerDto::Semantic { threshold: 0.9, score: Some(1.0) };
    ...
    fn semantic_trigger_rejects_below_threshold() {
        ...
        let trigger = MemoryTriggerDto::Semantic { threshold: 0.5, score: None };
    ...
    fn compute_matched_triggers_labels_semantic_correctly() {
        ...
        let behavior = sample_behavior(
            vec![MemoryTriggerDto::Semantic { threshold: 0.9, score: Some(1.5) }],
