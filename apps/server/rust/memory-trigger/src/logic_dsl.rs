...
fn get_path_values(root: &Value, path: &str) -> Vec<Value> {
    ...
    let segments =
        path.split('.').map(str::trim).filter(|segment| !segment.is_empty()).collect::<Vec<_>>();
...
fn evaluate_leaf(expr: &MemoryLogicExprDto, root: &Value) -> bool {
    match expr {
        MemoryLogicExprDto::Eq { path, value } => {
            get_path_values(root, path).iter().any(|actual| matches_eq(actual, value))
        }
        MemoryLogicExprDto::In { path, values } => {
            get_path_values(root, path).iter().any(|actual| matches_in(actual, values))
        }
        MemoryLogicExprDto::Gt { path, value } => get_path_values(root, path)
            .iter()
            .any(|actual| to_number(actual).is_some_and(|candidate| candidate > *value)),
        MemoryLogicExprDto::Lt { path, value } => get_path_values(root, path)
            .iter()
            .any(|actual| to_number(actual).is_some_and(|candidate| candidate < *value)),
        MemoryLogicExprDto::Contains { path, value } => {
            get_path_values(root, path).iter().any(|actual| {
                if let Some(inner) = actual.as_str() {
                    return inner.contains(value);
                }

                if let Some(array) = actual.as_array() {
                    return array.iter().any(|item| to_comparable_string(item).contains(value));
                }
        ...
        MemoryLogicExprDto::And { .. }
        | MemoryLogicExprDto::Or { .. }
        | MemoryLogicExprDto::Not { .. } => false,
    }
}

pub fn evaluate_memory_logic_expr(expr: &MemoryLogicExprDto, root: &Value) -> bool {
    match expr {
        MemoryLogicExprDto::And { items } => {
            items.iter().all(|item| evaluate_memory_logic_expr(item, root))
        }
        MemoryLogicExprDto::Or { items } => {
            items.iter().any(|item| evaluate_memory_logic_expr(item, root))
        }
...
mod tests {
    ...
    fn evaluates_contains_and_exists() {
        ...
        let exists_expr =
            MemoryLogicExprDto::Exists { path: "recent.event.0.payload.title".to_string() };
