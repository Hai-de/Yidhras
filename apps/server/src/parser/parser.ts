...
import type { AstNode, BlockNode, MacroNode, ModifierSpec, ParserDiagnostic, Token, VariableNode } from './types.js'

const parseModifierChain = (
  expression: string,
  syntax: ParserSyntaxConfig
  ...
  const parseNodes = (stopKeyword?: string): AstNode[] => {
    ...
    while (hasMore()) {
      ...
      if (token.type === 'VAR_OPEN') {
        advance()
        const parts: string[] = []
        while (hasMore() && peek()!.type !== 'VAR_CLOSE') {
          const inner = advance()
          if (inner.type === 'TEXT' && inner.content) {
            parts.push(inner.content)
          }
        }
        if (hasMore() && peek()!.type === 'VAR_CLOSE') {
          advance()
        }
      ...
      if (token.type === 'MACRO_OPEN') {
        advance()
        const parts: string[] = []
        while (hasMore() && peek()!.type !== 'MACRO_CLOSE') {
          const inner = advance()
          if (inner.type === 'TEXT' && inner.content) {
            parts.push(inner.content)
          }
        }
        if (hasMore() && peek()!.type === 'MACRO_CLOSE') {
          advance()
        }
        const expression = parts.join('')
      ...
      if (token.type === 'BLOCK_OPEN') {
        ...
        if (
          nextToken?.type === 'TEXT' &&
          nextToken.content?.trim() === syntax.blocks.conditional.elseKeyword &&
          stopKeyword
        ) {
          break
        }
        ...
        if (peek()?.type === 'BLOCK_OPEN') {
          ...
          if (
            la?.type === 'TEXT' &&
            la.content?.trim() === syntax.blocks.conditional.elseKeyword &&
            la2?.type === 'MACRO_CLOSE'
          ) {
            advance()
            advance()
            advance()
