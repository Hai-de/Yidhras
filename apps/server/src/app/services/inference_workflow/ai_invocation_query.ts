...
import { ensureNonEmptyId, parseOptionalCreatedAtFilter } from './parsers.js';
...
export interface AiInvocationListItem {
  ...
  status: (typeof AI_INVOCATION_STATUSES)[number];
...
export interface AiInvocationsListSnapshot {
  ...
  summary: {
    returned: number;
    limit: number;
    counts_by_status: Record<(typeof AI_INVOCATION_STATUSES)[number], number>;
    filters: {
      status: string[] | null;
...
interface ParsedAiInvocationFilters {
  ...
  cursor: AiInvocationCursor | null;
  limit: number;
}
...
const parseAiInvocationListLimit = (value: number | undefined): number => {
  const requested = typeof value === 'number' && Number.isFinite(value) ? Math.trunc(value) : DEFAULT_AI_INVOCATION_LIST_LIMIT;
  return Math.min(MAX_AI_INVOCATION_LIST_LIMIT, Math.max(1, requested));
};
...
const parseAiInvocationStatuses = (value: string[] | undefined): string[] | null => {
  ...
  const invalid = normalized.filter(item => !(AI_INVOCATION_STATUSES as readonly string[]).includes(item));
  if (invalid.length > 0) {
...
const parseAiInvocationCursor = (value: string | undefined): AiInvocationCursor | null => {
  ...
  if (
    ...
    !/^\d+$/.test((parsed as { created_at: string }).created_at) ||
  ...
  return {
    created_at: (parsed as { created_at: string }).created_at,
...
const parseAiInvocationFilters = (input: ListAiInvocationsInput): ParsedAiInvocationFilters => {
  ...
  return {
    ...
    provider: parseOptionalFilterId(input.provider),
    model: parseOptionalFilterId(input.model),
    task_type: parseOptionalFilterId(input.task_type),
    source_inference_id: parseOptionalFilterId(input.source_inference_id),
    route_id: parseOptionalFilterId(input.route_id),
...
const compareCursorPosition = (left: { created_at: string; id: string }, right: { created_at: string; id: string }): number => {
...
const matchesCursor = (cursor: AiInvocationCursor | null, item: AiInvocationListItem): boolean => {
  if (!cursor) {
    return true;
  }

  return compareCursorPosition({ created_at: item.created_at, id: item.id }, cursor) > 0;
};

const buildWhere = (filters: ParsedAiInvocationFilters): Prisma.AiInvocationRecordWhereInput => {
  return {
    ...
    ...(filters.has_error === null ? {} : filters.has_error ? { error_code: { not: null } } : { error_code: null }),
...
const toAttemptedModels = (value: unknown): string[] => {
  return Array.isArray(value) ? value.filter((item): item is string => typeof item === 'string') : [];
};

const toListItem = (record: AiInvocationRecord): AiInvocationListItem => {
  const normalizedStatus = (AI_INVOCATION_STATUSES as readonly string[]).includes(record.status)
    ? (record.status as (typeof AI_INVOCATION_STATUSES)[number])
    : 'failed';
  ...
  return {
    ...
    attempted_models: toAttemptedModels(record.attempted_models_json),
...
export const listAiInvocations = async (
  ...
  const filteredItems = records.map(record => toListItem(record)).filter(item => matchesCursor(filters.cursor, item));
  const hasNextPage = filteredItems.length > filters.limit;
  const pageItems = hasNextPage ? filteredItems.slice(0, filters.limit) : filteredItems;

  const countsByStatus: Record<(typeof AI_INVOCATION_STATUSES)[number], number> = {
    completed: 0,
    failed: 0,
    blocked: 0,
    timeout: 0
