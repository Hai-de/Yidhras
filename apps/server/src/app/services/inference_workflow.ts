...
import type { InferenceActionIntentSnapshot, InferenceJobSubmitResult, InferenceRequestInput, WorkflowSnapshot } from '../../inference/types.js';
...
const buildWorkflowSnapshotBundleForJobs = async (
  ...
  const [traces, intents, replayParents, replayChildren] = await Promise.all([
    traceIds.length > 0 ? context.prisma.inferenceTrace.findMany({ where: { id: { in: traceIds } } }) : Promise.resolve([]),
    intentIds.length > 0 ? context.prisma.actionIntent.findMany({ where: { id: { in: intentIds } } }) : Promise.resolve([]),
    replayParentIds.length > 0 ? context.prisma.decisionJob.findMany({ where: { id: { in: replayParentIds } } }) : Promise.resolve([]),
    jobIds.length > 0
      ? context.prisma.decisionJob.findMany({
          where: { replay_of_job_id: { in: jobIds } },
  ...
  return {
    traceByInferenceId: new Map(traces.map(trace => [trace.id, trace])),
    intentById: new Map(intents.map(intent => [intent.id, intent])),
    replayParentJobById: new Map(replayParents.map(job => [job.id, job])),
