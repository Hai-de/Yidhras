...
export const reinitializePackRuntime = async (input: ReinitializePackRuntimeInput): Promise<void> => {
  const { activePackRuntime, packFolderName, packId, openingId, prisma, packStorageAdapter, notifications } = input;
  ...
  await activePackRuntime.init(packFolderName, openingId);
