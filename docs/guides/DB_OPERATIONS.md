...
本文档面向 **部署者 / 运维 / 本地环境维护者**，说明 Yidhras 服务端如何：
...
后端使用 Prisma，支持 SQLite 和 PostgreSQL 两种后端，通过 `PRISMA_DB_PROVIDER` 环境变量切换（默认 `sqlite`）。
...
> seed 是“确保基础记录存在”的思路，不是面向生产多租户/复杂回填的通用数据导入器。
...
> 注意：`reset:dev-db` 针对的是默认工作区数据库文件 `data/yidhras.sqlite`，不是任意 `DATABASE_URL` 目标。
...
- `reset_dev_db.ts` 仅操作默认 SQLite 文件路径，不覆盖 PostgreSQL 数据库
