...
Pack 运行时数据库已通过 `PackStorageAdapter` 接口抽象。当前主要后端为 SQLite，另有实验性 PostgreSQL 实现：
...
- `SchedulerStorageAdapter` 当前仅有 SQLite 实现
...
> **注意**：Pack-local 层的 PostgreSQL 支持为实验性（见 §2.4.2 限制说明），`SchedulerStorageAdapter` 仅 SQLite 实现。生产环境建议从 SQLite 起步。
