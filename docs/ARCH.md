...
| 实现 | 后端 | 状态 | 隔离策略 |
|------|------|------|----------|
| `SqlitePackStorageAdapter` | SQLite（`node:sqlite`） | 生产可用 | 每 pack 独立文件：`data/world_packs/<packId>/runtime.sqlite` |
...
- `PackRuntimePort` / `DefaultPackRuntimePort`（`app/services/pack_runtime_ports.ts`, `packs/orchestration/default_pack_runtime_port.ts`）
  - 每个已加载 pack 的 per-pack 运行时端口，替代旧的单一 `activePackRuntime`。提供 `getCurrentTick()`、`getCurrentRevision()`、`getPack()`、`step()` 等方法
- `pack_runtime_resolution.ts`（`app/services/pack_runtime_resolution.ts`）
  - 统一 tick/revision/activePack 解析入口。优先从传入的 `packRuntime` 参数获取，其次通过 `packRuntimeLookup` → `getPackRuntimeHost`，最后回退兼容旧 stub
...
- "Active pack" 概念退化为纯粹的路由约定：stable plugin surface（`/api/packs/:packId/plugins/...`）仅校验当前活跃包，experimental surface（`/api/experimental/runtime/packs/:packId/...`）不限制

已删除的旧概念：
- `PackRuntimeScopeMode = 'stable' | 'experimental'` — 已删除，所有 pack 统一走 registry-based 解析
- `DefaultActivePackRuntimeFacade` — 已删除，由 `PackRuntimePort` / `pack_runtime_resolution.ts` 替代

对等架构下的路由分层：
- 所有 pack-scoped 路由通过统一的 `/:packId` 前缀访问
- `packScopeMiddleware` 从 URL param 解析 packId，通过 `PackScopeResolver` 做状态门控
- Stable surface（plugin web 等）在路由 handler 内做 active-pack 匹配校验
...
- `pack_runtime_resolution.ts`
  - 服务层统一解析入口：`resolvePackTick(context, packRuntime?)`, `resolveActivePack(context, packRuntime?)` 等
- `PackScopeResolver`
...
- Stable plugin runtime surface（`/api/packs/:packId/plugins/runtime/web`）保留 active-pack 校验，不因多包运行时自动放宽
