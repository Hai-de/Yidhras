...
apps/server/src/app/services/repositories/
  ...
  SchedulerRepository.ts            # SchedulerRun / CandidateDecision (Lease/Cursor 已迁至 SchedulerStorageAdapter)
...
`AppInfrastructure.repos: Repositories` 在组合根注入。调用方通过 `context.repos.<domain>.method()` 访问，替代旧的 `context.prisma.model.operation()` 直接调用。

**迁移状态**：Scheduler、Identity/Operator 等核心调用链已完成迁移；~32 个文件仍在使用 `context.prisma`，分阶段持续推进。
...
当前 `scheduler_lease.ts` 已完全通过 adapter 读写，不再依赖 Prisma。`scheduler_ownership.ts`、`scheduler_rebalance.ts`、`scheduler_observability.ts` 仍通过 `context.repos.scheduler` 走 Prisma，待后续迁移。
...
- `apps/server/src/app/runtime/*.ts`
  ...
  - `simulation_loop.ts` — 旧全局单循环（兼容 active pack，逐步被 `MultiPackLoopHost` 取代）
...
- `createRuntimeKernelService(context, packId?)`
  - 当前 Node/TS 宿主内的 runtime kernel service 实现，接受可选 `packId` 参数
...
Scheduler Docker 式隔离重构后，多 pack 调度已从实验性特性提升为默认架构：
...
- `SimulationManager`
  - 仅作为兼容 facade，通过 `setMultiPackLoopHost()` 注入 loop host
